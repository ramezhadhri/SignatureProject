...
export default {
  ...
  computed: {
    filteredDemandes() {
      const filtered = this.demandes.filter((demande) => {
        const dateCondition =
          (!this.fromDate || demande.date >= this.fromDate) &&
          (!this.toDate || demande.date <= this.toDate);

        const statusCondition =
          this.selectedStatus === "Etat" || demande.etat === this.selectedStatus;
    ...
    paginatedDemandes() {
      ...
      const endIndex = startIndex + this.itemsParPage;
      return this.filteredDemandes.slice(startIndex, endIndex);
    },
    totalPages() {
      return Math.ceil(this.filteredDemandes.length / this.itemsParPage);
    },
  },
  methods: {
    viewDocument(id) {
